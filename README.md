...
| `CC_LANGFUSE_CAPTURE_IMAGES` | Upload images to Langfuse and show them in the trace (default `true`). This includes pasted images and screenshots from tool results. Media upload must be available on your Langfuse deployment (self-hosted: `LANGFUSE_S3_MEDIA_UPLOAD_*`); if it is not, set this option to `false`. If the option is `false`, the trace shows a marker for each image, for example `[image image/png ~200KB]`. The hook then also sets `LANGFUSE_MEDIA_UPLOAD_ENABLED=false` to stop automatic uploads by the SDK. If you set that variable yourself, the hook does not change it. Do not set `LANGFUSE_MEDIA_UPLOAD_ENABLED=false` while capture is on: the trace then shows image references without content. |
...
| `CC_LANGFUSE_SYNC_SESSION_END` | Optional. Keep the `SessionEnd` upload in the foreground (default false). At the session end the hook normally hands the upload to a background process, because the CLI stops a foreground hook about 1.5 seconds after the exit. Set this to `true` to make the CLI wait for the upload and report its failures, for example while you debug a missing final turn. |
...
| `SessionEnd has work to do; a child process completes it`, and the final turn never arrives | The background process that uploads the last turn ended before it finished. Its errors reach this log only | Set `CC_LANGFUSE_SYNC_SESSION_END=true` and repeat the session. The CLI then waits for the upload and shows the error, at the price of a slower exit |
