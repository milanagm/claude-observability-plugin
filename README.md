...
| `CC_LANGFUSE_CAPTURE_IMAGES` | Set to `true` to upload images to Langfuse and show them in the trace (default `false`). This includes pasted images and screenshots from tool results. Media upload must be available on your Langfuse deployment (self-hosted: `LANGFUSE_S3_MEDIA_UPLOAD_*`). If the option is `false`, the trace shows a marker for each image, for example `[image image/png ~200KB]`. The hook then also sets `LANGFUSE_MEDIA_UPLOAD_ENABLED=false` to stop automatic uploads by the SDK. If you set that variable yourself, the hook does not change it. Do not set `LANGFUSE_MEDIA_UPLOAD_ENABLED=false` together with capture on: the trace then shows image references without content. |
